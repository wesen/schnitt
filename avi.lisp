...
(defun copy-avi (&key avi-hdr avi-vid-strh avi-vid-strf avi-aud-strh avi-aud-strf
		 vid-frames aud-frames default)
  (make-instance 'avi
		 :avi-hdr (copy-object (or avi-hdr (when default
						     (avi-hdr default))))
		 :avi-vid-strh (copy-object (or avi-vid-strh (when default
						     (avi-vid-strh default))))
		 :avi-vid-strf (copy-object (or avi-vid-strf (when default
						     (avi-vid-strf default))))
		 :avi-aud-strh (copy-object (or avi-aud-strh (when default
						     (avi-aud-strh default))))
		 :avi-aud-strf (copy-object (or avi-aud-strf (when default
							       (avi-aud-strf default))))
		 :vid-frames (copy-list (or vid-frames (when default
							 (vid-frames default))))
		 :aud-frames (copy-list (or aud-frames (when default
							 (aud-frames default))))))

(defmethod print-object ((avi avi) stream)
  (print-unreadable-object (avi stream :type t :identity t)
    (format stream "~A frames" (length (vid-frames avi)))))
...
(defun make-riff-list (type chunks)
  (make-instance 'riff-list :id "LIST" :list-type type :chunks chunks :size
		 (+ 20 (loop for chunk in chunks
			  summing (size chunk)))))
...
(defun make-idx1 (frames)
  (let* ((entries (loop with pos = 224
			...
			collect (make-instance 'idx1-entry :id (id frame)
					       ...
					       :pos pos)
			do (incf pos (+ 8 (size frame))))))
    (make-instance 'idx1-chunk :id "idx1" :size 48 :entries entries)))
...
(defmethod make-avi-file ((avi avi))
  (let* ((hdr (make-instance 'riff-hdr :id "RIFF" :riff-type "AVI " :size 0))
	 ...
	 (idx1 (make-idx1 frames)))
    (incf (size movi) 8) ; ??X 
    (setf (total-frames avi-hdr) (length frames))
    (setf (size hdr) (+ 28 (+ (size hdrl)
			      (size movi)
			      (size idx1))))
    (make-instance 'avi-file
		   :hdr  hdr
		   :hdrl hdrl
		   :chunks (list movi idx1))))
